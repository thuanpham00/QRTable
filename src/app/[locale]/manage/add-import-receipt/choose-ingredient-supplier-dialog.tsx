...
const getColumns = (t: (key: string) => string): ColumnDef<SupplyItem>[] => [
  ...
  },
  {
    ...
    header: () => <div className="text-right">{t("unit")}</div>,
    size: 100,
    cell: ({ row }) => <div className="text-right">{row.original.ingredient?.unit || "—"}</div>,
  },
  {
    accessorKey: "isPreferred",
    header: () => <div className="text-right">{t("preferred")}</div>,
    size: 100,
    cell: ({ row }) => (
  ...
  {
    accessorKey: "note",
    header: () => <div className="text-right">{t("note")}</div>,
    size: 200,
    cell: ({ row }) => (
      <div className="text-xs text-muted-foreground truncate max-w-50 text-right">
        {row.getValue("note") || "—"}
      </div>
    ),
  },
  ...
  return (
    <Dialog open={showModalChooseIngredient} onOpenChange={setShowModalChooseIngredient}>
      <DialogTrigger asChild>
        <Button size="sm" className="h-7 gap-1 w-40" disabled={disable}>
