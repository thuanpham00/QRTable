...
import InventoryStockTable from "@/app/[locale]/manage/inventory-stocks/inventory-stock-table";
...
export async function generateMetadata(props: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({
    ...
    namespace: "ManageInventoryStocks",
  });

  const url = envConfig.NEXT_PUBLIC_URL + `/${params.locale}/manage/inventory-stocks`;
...
export default async function InventoryStockPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations("ManageInventoryStocks");
